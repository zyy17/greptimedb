...
pub fn init_global_logging(
    ...
    START.call_once(|| {
        ...
        // Build the stdout logging layer.
        let stdout_logging_layer = {
            if logging_opts.append_stdout {
        ...
        // Build the file logging layer.
        let file_logging_layer = {
            let rolling_appender =
                RollingFileAppender::new(Rotation::HOURLY, &logging_opts.dir, app_name);
            let (writer, guard) = tracing_appender::non_blocking(rolling_appender);
            guards.push(guard);
            Layer::new().with_writer(writer).with_ansi(false)
        };

        // Build the error file logging layer.
        let error_file_logging_layer = {
            let filename_prefix = format!("{}-err", app_name);
            let rolling_appender =
                RollingFileAppender::new(Rotation::HOURLY, &logging_opts.dir, filename_prefix);
            let (writer, guard) = tracing_appender::non_blocking(rolling_appender);
            guards.push(guard);
            Layer::new().with_writer(writer).with_ansi(false)
        };
        ...
        let filter = {
            ...
            // - default settings
            let rust_log_env = env::var(EnvFilter::DEFAULT_ENV).ok();
            let targets_string = logging_opts
                .level
                .as_deref()
        ...
        let subscriber = Registry::default()
            ...
            .with(stdout_logging_layer.map(|x| x.with_filter(filter.clone())))
            .with(file_logging_layer.with_filter(filter))
            .with(error_file_logging_layer.with_filter(filter::LevelFilter::ERROR));
        ...
        if logging_opts.enable_otlp_tracing {
            global::set_text_map_propagator(TraceContextPropagator::new());

            let sampler = logging_opts
